...
const ModalAddress = () => {
  ...
  const { entityId, entityName } = location.state;
  console.log(entityId, "this is eid")
  ...
  const validation = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        console.log(values);
        history.push("/address-table", { entityId, entityName });
      } catch (error) {
        console.log(error);
      }
    },
  ...
  const fetchZoneAndRegion = async () => {
    try {
      const [zoneResponse, regionResponse, TypeResponse] = await Promise.all([
                  ...
                  <Row>
                    <Col className="col-7">
                      <div className="text-primary p-4">
                        <h5 className="text-primary">Add Address Details</h5>
                      </div>
                    </Col>
